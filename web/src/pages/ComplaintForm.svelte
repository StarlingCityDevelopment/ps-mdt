	...
	async function handleSubmit() {
		...
		try {
			...
			const resp = await fetch(
				`https://${resourceName}/submitComplaint`,
				{
					method: "POST",
					headers: {
			...
			{#if submitted}
				...
				<div class="success-container">
					...
					<h2 class="success-title">
						{$t("complaint.filed_successfully")}
					</h2>
					<p class="success-number">
						{$t("complaint.your_complaint_number")}:
						<strong>{complaintNumber}</strong>
					</p>
					<p class="success-note">
						{$t("complaint.contact_if_needed")}
					</p>
				</div>
			{:else}
				<div class="card-header">
					...
					<h2 class="header-title">
						{$t("complaint.internal_affairs_complaint")}
					</h2>
				</div>
				...
				>
					...
					<div class="form-group">
						<label class="form-label" for="officerName"
							>{$t("complaint.officer_name")}
						...
						<input
							...
							class="form-input"
							placeholder={$t(
								"complaint.officer_name_placeholder",
							)}
					...
					<!-- Officer Badge -->
					<div class="form-group">
						<label class="form-label" for="officerBadge"
							>{$t("complaint.officer_badge")}</label
						>
						<input
							id="officerBadge"
							type="text"
							class="form-input"
							placeholder={$t("complaint.badge_number")}
					...
					<div class="form-group">
						<label class="form-label" for="incidentDate"
							>{$t("complaint.incident_date")}</label
						>
						<input
							id="incidentDate"
							type="date"
					...
					<!-- Location -->
					<div class="form-group">
						<label class="form-label" for="incidentLocation"
							>{$t("complaint.location")}</label
						>
						<input
							id="incidentLocation"
							type="text"
					...
					<div class="form-group">
						...
						<textarea
							...
							rows="5"
							placeholder={$t(
								"complaint.description_placeholder",
							)}
					...
					<!-- Witnesses -->
					<div class="form-group">
						<label class="form-label" for="witnesses"
							>{$t("complaint.witnesses")}</label
						>
						<textarea
							id="witnesses"
							class="form-input form-textarea"
							rows="2"
							placeholder={$t("complaint.witnesses_placeholder")}
					...
					<div class="form-group">
						...
						<div class="evidence-input-row">
							<input
								...
								placeholder={$t(
									"complaint.evidence_url_placeholder",
								)}
								...
								{#each evidenceList as item, i}
									<li class="evidence-item">
										<span
											...
											title={item.url}>{item.label}</span
										>
										<button
											type="button"
											class="btn-remove-evidence"
